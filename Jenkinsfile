/*
  Jenkinsfile — Linux-style steps (sh). Minimal change: add Helm --wait --atomic and increase timeouts to avoid rollout timing out.
*/
...
pipeline {
  ...
  environment {
    ...
    HELM_TIMEOUT   = '180s'  // increased from 120s
  }
  ...
  stages {
    ...
    stage('Smoke Test') {
      steps {
        sh '''
          set -e
          kubectl -n ${K8S_NAMESPACE} rollout status deploy/${APP_NAME} --timeout=${HELM_TIMEOUT}
          NODEPORT=$(kubectl -n ${K8S_NAMESPACE} get svc ${APP_NAME} -o jsonpath="{.spec.ports[0].nodePort}")
          IP=$(minikube ip)
          # Try for up to 60s (12 x 5s) in case app needs a brief warm-up
          for i in $(seq 1 12); do
            if curl -sf "http://${IP}:${NODEPORT}/health" > /dev/null; then
